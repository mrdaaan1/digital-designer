...
- КРИТИЧНО для sourceSlideIndex: генератор клонирует XML именно этого слайда
  ...
  на этом слайде, в порядке их появления в XML сверху вниз. Поэтому:
  * Список placeholderRoles должен содержать РОВНО столько ролей, сколько
    отдельных текстовых блоков реально на слайде sourceSlideIndex (не больше,
    не меньше) — включая мелкие подписи, лейблы, метрики, если они там есть.
  ...
  * Порядок ролей в placeholderRoles должен точно соответствовать порядку
    текстовых блоков в XML/на рендере (сверху вниз, слева направо).
